...
local function capture_current_window()
    ...
    if not ui.is_plugin_window(current_win) and vim.api.nvim_win_is_valid(current_win) then
        ...
        log.debug("tabs.capture_current_window: Remembered active window: " .. tostring(last_real_win))
    end
end
...
local function update_tabs_state_in_db()
    if not config.autosave or not state.workspace_id then
        log.debug("tabs.update_tabs_state_in_db: Autosave disabled or no workspace_id. Skipping.")
        return
    end
    ...
    if ws then
        ...
        log.debug("tabs.update_tabs_state_in_db: Updating workspace tabs data: " .. vim.inspect(tabs_json_obj))
        data.update_workspace_tabs(vim.fn.json_encode(tabs_json_obj), state.workspace_id)
    else
        log.warn(
            "tabs.update_tabs_state_in_db: Could not find workspace " .. state.workspace_id .. " to update tabs state."
        )
    end
end
...
local function add_tab_db(tab_name_input, workspace_id)
    ...
    if not new_tab_id or type(new_tab_id) ~= "number" or new_tab_id <= 0 then
        log.error("add_tab_db: Failed to add tab to database. DB returned: " .. tostring(new_tab_id))
        return (tab_def and tab_def.add_failed) or "TAB_ADD_FAILED"
    end

    log.info(
        string.format("add_tab_db: Tab '%s' (ID: %d) added to workspace %d.", tab_name_input, new_tab_id, workspace_id)
    )

    local current_ws = data.find_workspace_by_id(workspace_id, state.project_id)
    if not current_ws then
        log.error("add_tab_db: Could not find workspace " .. workspace_id .. " to update its tab list.")
    ...
    if not update_success then
        log.error("add_tab_db: Failed to update workspace " .. workspace_id .. " with new tab ID list.")
        data.delete_tab(new_tab_id, workspace_id)
        return (tab_def and tab_def.add_failed) or "TAB_ADD_FAILED"
    end
...
local function rename_tab_db(tab_id, new_tab_name_validated, workspace_id, selected_line_num)
    ...
    log.info(string.format("rename_tab_db: Tab ID %s renamed to '%s'", tab_id, new_tab_name_validated))
...
local function delete_tab_db(tab_id_to_delete, workspace_id, selected_line_num)
    ...
    if not success then
        log.warn("delete_tab_db: Failed to delete tab ID " .. tab_id_to_delete)
        return nil
    end
    log.info("delete_tab_db: Tab ID " .. tab_id_to_delete .. " deleted successfully")

    vim.schedule(function()
        local current_ws = data.find_workspace_by_id(workspace_id, state.project_id)
        if not current_ws then
            log.error("delete_tab_db: Could not find workspace " .. workspace_id .. " to update its tab list.")
            M.init(selected_line_num)
            return
        end
        ...
        if index_to_remove then
            ...
            log.debug("delete_tab_db: Removed tab ID " .. tab_id_to_delete .. " from workspace tab_ids list.")
        end
        ...
        if was_active_tab then
            ...
            log.info("delete_tab_db: Cleared active state as deleted tab was active.")
        end
        ws_tabs_data.updated_at = os.time()
...
local function handle_move_operation(ctx, direction)
    if not ctx or not ctx.win or not vim.api.nvim_win_is_valid(ctx.win) then
        log.warn("tabs.handle_move_operation: Invalid UI context or window.")
        return
    end
    ...
    if not tab_id_to_move then
        log.warn("tabs.handle_move_operation: Could not determine tab at current line (" .. current_visual_line .. ").")
        return
    end
    ...
    local tab_def = get_entity_def()
    if not tab_def then
        log.error("tabs.handle_move_operation: 'tab' entity type definition not found in common.lua.")
    ...
    if not tab_to_move_data then
        log.error(
            string.format(
                "tabs.handle_move_operation: Cache inconsistency for line %d. ID from map: %s, data not found for ID.",
                current_visual_line,
                tostring(tab_id_to_move)
            )
        )
    ...
    if not current_sort_order then
        log.error(
            "tabs.handle_move_operation: tab_to_move_data.sort_order is not a number: "
                .. tostring(tab_to_move_data.sort_order)
        )
    ...
    log.debug("tabs.handle_move_operation: Reordering tabs with table: " .. vim.inspect(new_order_table))
    local success, err_msg_code = data.reorder_tabs(state.workspace_id, new_order_table)

    if success then
        log.info(
            string.format(
                "Tab ID %s ('%s') moved %s from sort_order %d to %d in workspace %s.",
                tab_id_to_move,
                tab_to_move_data.name,
                direction,
                current_sort_order,
                target_sort_order,
                state.workspace_id
        ...
        log.error(
            string.format(
                "tabs.handle_move_operation: Error reordering tab ID %s in workspace %s: Code: %s",
                tab_id_to_move,
                state.workspace_id,
...
function M.handle_tab_add()
    local tab_def = get_entity_def()
    if not tab_def then
        log.error("handle_tab_add: 'tab' entity type definition not found.")
...
function M.handle_tab_rename(ctx)
    ...
    if not tab_id_to_rename then
        log.warn("handle_tab_rename: No tab ID found at cursor line " .. current_line_num)
        return
    end
...
function M.handle_tab_delete(ctx)
    ...
    if not tab_id_to_delete then
        log.warn("handle_tab_delete: No tab ID found at cursor line " .. current_line_num)
        return
    end
...
function M.handle_tab_switch(opts)
    ...
    if not tab_id_selected then
        log.warn("tabs.handle_tab_switch: No tab selected from list (ID at cursor is nil).")
        return
    end

    if tostring(state.tab_active) == tostring(tab_id_selected) then
        log.info("tabs.handle_tab_switch: Already in tab ID: " .. tostring(tab_id_selected))
    ...
    vim.defer_fn(function()
        if success then
            log.info("tabs.handle_tab_switch: Successfully switched to tab " .. tostring(tab_id_selected))
...
function M.navigate_to_files()
    ...
    if not state.tab_active then
        notify.info(state.lang[(tab_def and tab_def.not_active) or "TAB_NOT_ACTIVE"] or "No active tab.")
...
M.init = function(selected_line_num)
    capture_current_window()
    log.debug(
        "tabs.M.init: Initializing tabs list for workspace_id: "
    ...
    if ws_data_ok and workspace_obj and workspace_obj.name then
        ...
        log.warn(
            "tabs.M.init: Could not retrieve current workspace name for workspace_id: "
    ...
    if not actual_selected_line and state.tab_active then
        for i, tab_entry in ipairs(cache.tabs_from_db) do
            if tostring(tab_entry.id) == tostring(state.tab_active) then
                ...
                log.debug("tabs.M.init: Found active tab ID " .. state.tab_active .. " at line " .. i)
                break
            end
        end
    end
    ...
    if not ctx_new then
        log.error("tabs.M.init: common.init_entity_list returned no context. UI creation likely failed.")
        return
    end
    ...
    log.debug("tabs.M.init: Tabs list initialized successfully.")
end
...
M.switch_to_tab_by_name = function(tab_name, workspace_id_context)
    ...
    if not target_workspace_id then
        log.warn("switch_to_tab_by_name: No target_workspace_id provided or in state.")
        return false
    end
    log.debug(
        "switch_to_tab_by_name: Attempting to switch to tab '" .. tab_name .. "' in workspace " .. target_workspace_id
    )
    ...
    if found_tab then
        log.debug("switch_to_tab_by_name: Found tab ID: " .. found_tab.id)
        update_tabs_state_in_db()
        ...
        log.warn(
            "switch_to_tab_by_name: Tab with name '" .. tab_name .. "' not found in workspace " .. target_workspace_id
        )
