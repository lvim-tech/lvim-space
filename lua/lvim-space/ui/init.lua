...
M.create_window = function(options)
	...
	-- Validate dimensions
	if not win_config.width or win_config.width < 1 or not win_config.height or win_config.height < 1 then
		safe_delete_buf(buf)
		return nil, nil
	end
...
M.open_main = function(lines, name, selected_line)
	...
	local main_config = config.ui.border.main or {}
	...
	local buf, win = M.create_window({
		...
		on_create = function(win, buf)
			...
			vim.schedule(function()
				if is_valid_win(win) then
					api.nvim_win_set_cursor(win, { selected_line, 0 })
				end
			end)
		end,
	})
...
function M.submit_input()
	...
	if is_valid_buf(buf) then
		...
		callback = api.nvim_buf_get_var(buf, "input_callback")
	end
